...
using MantisNetMvc.Infrastructure.Services;
using MantisNetMvc.Web.Filters;

var builder = WebApplication.CreateBuilder(args);

// Add Database Context (Defaulting to In-Memory / SQLite / LocalDB for high portability)
builder.Services.AddDbContext<MantisDbContext>(options =>
{
    var connectionString = builder.Configuration.GetConnectionString("DefaultConnection") 
        ?? "Server=(localdb)\\mssqllocaldb;Database=MantisNetDb;Trusted_Connection=True;MultipleActiveResultSets=true";
...
.AddEntityFrameworkStores<MantisDbContext>()
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        var context = services.GetRequiredService<MantisDbContext>();
    ...
    catch (Exception ex)
    {
        ...
        logger.LogError(ex, "An error occurred while seeding the MantisNet database.");
    }
}
